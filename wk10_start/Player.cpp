...
Player::Player(char* name, int x, int y, int z) : MD2Model()
{
	...
	m_score = 0;
	m_lives = 3;
	m_hasFlag = m_bbRender = m_isMoving = m_isSprinting = m_isWalking = m_isJumping = false;
	m_firstTime = true;
}
...
{
	float rotAngle = acos((m_velocity.UnitVector()).DotProduct(m_direction.UnitVector()));
	rotAngle = rotAngle * 180.0f/float(M_PI);

	Vector rotAxis = (m_direction.UnitVector()).CrossProduct(m_velocity.UnitVector());
	rotAxis.Normalize();
